...
def main_evaluation(args,default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True):
    ...
    if args.norm_36_char: 
        if resDict['e2e_method'] is not None: 
            print(f"E2E Score with 36 character: {resDict['e2e_method']}")

    else: 
        if resDict['e2e_method'] is not None: 
            print(f"E2E Score (case-{args.case}): {resDict['e2e_method']}")

    print(f"Detection Score: {resDict['det_only_method']}")
