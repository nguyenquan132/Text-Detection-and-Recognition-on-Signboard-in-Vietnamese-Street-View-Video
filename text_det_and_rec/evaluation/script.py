"""
This file is adapted from the TEDEval repository.
Original source: https://github.com/naver/TEDEval
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        for n in range(len(pointsList)):
            points = pointsList[n]
            transcription = '###' if transcriptionsList[n] in ['#', '##', '###'] else transcriptionsList[n]
        ...
        if resFile in subm:
            ...
            for n in range(len(pointsList)):
                points = pointsList[n]
                transcription = transcriptionsList[n]
                if NULL_CHAR in transcription: 
                    transcription = str(transcription).replace(NULL_CHAR, '')
                            ...
                            if match:
                                ...
                                if normDist < evaluationParams['EV_PARAM_IND_CENTER_DIFF_THR'] :
                                    if evaluationParams['WORD_SPOTTING']: 
                                        if evaluationParams['NORM_36_CHAR']: 
                                            gt_text_wo_acc = mapping_without_accent(gtTrans[gtNum])
                                            pred_text_wo_acc = mapping_without_accent(detTrans[detNum])
                                            gt_text_wo_acc_norm = normalize_text(gt_text_wo_acc)
                                            pred_text_wo_acc_norm = normalize_text(pred_text_wo_acc)
                                            correct = (
                                                gt_text_wo_acc_norm
                                                == pred_text_wo_acc_norm
                                            )
                                            detCorrect += 1 if correct else 0
                                            if correct: 
                                                detMatchedNums.append(detNum)
                                        elif evaluationParams['CASE'] == 'insensitive':
                                            gt_text = str(gtTrans[gtNum]).lower()
                                            pred_text = str(detTrans[detNum]).lower()
                                            correct = (
                                                gt_text
                                                == pred_text
                                            )
                                            ...
                                            if correct: 
                                                detMatchedNums.append(detNum)
                                        elif evaluationParams['CASE'] == 'sensitive': 
                                            gt_text = str(gtTrans[gtNum])
                                            pred_text = str(detTrans[detNum])
                                            correct = (
                                                gt_text
                                                == pred_text
                                            )
                                            ...
                                            print(f'Error: Please choose the right params')
    ...
    e2eRecall = 0 if numGlobalCareGt == 0 else float(matchedSum) / numGlobalCareGt
    e2ePrecision = (
        0 if numGlobalCareDet == 0 else float(matchedSum) / numGlobalCareDet
    )
    e2eHmean = (
        0
        if e2eRecall + e2ePrecision == 0
        else 2 * e2eRecall * e2ePrecision / (e2eRecall + e2ePrecision)
    )
    if evaluationParams['WORD_SPOTTING']: 
        methodMetrics = {
...
if __name__=='__main__':
    ...
    parse.add_argument('-g', type=str, required=True, default='',  help='ground truth path')
    parse.add_argument('-s', type=str, required=True, default='', help='prediction resutl path')
    parse.add_argument('-iou_threshold', type=float, required=False, default=0.4, help='iou threshold to match between gt and pred')
    parse.add_argument('-word_spotting', default=False, action='store_true', help='word spotting evaluation protocol')
    parse.add_argument('-norm_36_char', default=False, action='store_true', help='norm text with 36 character for evaluation protocol')
    parse.add_argument('-case', type=str, required=False,  default='insensitive', help='case sensitive or insensitve to return accuracy')
    parse.add_argument('-conf', default=False, action='store_true', help=' 	set True if result file has confidence')
    parse.add_argument('-trans', default=False, action='store_true', help='set True if result file has transcription')
    parse.add_argument('-o', type=str, required=False, default='', help='output path folder to save file result')
