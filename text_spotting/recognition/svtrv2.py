...
from text_spotting.recognition.OpenOCR.openrec.optimizer import build_optimizer
from text_spotting.recognition.OpenOCR.openrec.preprocess import transform
from text_spotting.recognition.OpenOCR.openrec.preprocess import create_operators
...
def get_state_dict(state_dict): 
    ...
    for p, v in state_dict.items(): 
        ...
        if 'encoder' in key: 
            new_state_dict[p] = v
        if 'decoder' in key:
            if 'gtc_decoder' in key:
                if 'embedding' in p or 'ques1_head' in p:
                    print(f"[SKIP] {p}") 
                    continue 
                else:
                    new_state_dict[p] = v
            if 'ctc_decoder' in key:
                if 'fc' in key:
                    print(f"[SKIP] {p}")  
                    continue  
                else:
...
class SVTRV2Finetuner(pl.LightningModule):
    def __init__(self, model, criterion=None, cfg_optimizer=None, cfg_lr_scheduler=None, train_dataloader=None, val_dataloader=None,
        ...
        super(SVTRV2Finetuner, self).__init__()
        self.model = model
    ...
    def compute_loss(self, batch, batch_idx): 
        ...
        if self.model.training: 
            ...
            loss = self.criterion.ctc_loss(preds['ctc_pred'], [None] + batch_tensor[-2:])['loss']
            loss_dict = {'loss': loss, 'ctc_loss': loss}
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % self.metrics_interval == 0:
            ...
            self.train_gt.clear()
            metrics = {'train_loss': loss_dict['loss'], 'train_ctc_loss': loss_dict['ctc_loss'], 'train_gtc_loss': loss_dict['gtc_loss'],
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_loss_dict['val_loss'].append(loss_dict['loss'])
        self.val_loss_dict['val_ctc_loss'].append(loss_dict['ctc_loss'])

        return {'val_loss': loss_dict['loss']}

    def on_validation_epoch_end(self):
        avg_val_loss = torch.stack(self.val_loss_dict['val_loss']).mean()
        avg_val_ctc_loss = torch.stack(self.val_loss_dict['val_ctc_loss']).mean()

        self.val_loss_dict['val_loss'].clear()
        self.val_loss_dict['val_ctc_loss'].clear()
        ...
        metrics = {'val_loss': avg_val_loss, 'val_ctc_loss': avg_val_ctc_loss,
                  'val_acc_strict': accuracy_strict, 'val_acc_lenient': accuracy_lenient}
        
        for k,v in metrics.items():
            self.log(k, v)

        return {'val_loss': avg_val_loss, 
               'val_acc_strict': accuracy_strict, 
...
def train(config, train_ops, val_ops, pretrained_model_path, max_epochs, device='cpu', seed=42):
    ...
    post_process_class = build_post_process(config['PostProcess'],
                                            config['Global'])
    post_process_class.only_gtc = True
    ...
    svtrv2_finetuner = SVTRV2Finetuner(model=model, criterion=criterion, cfg_optimizer=config['Optimizer'], 
    ...
    wandb_logger = WandbLogger(project='Text Recognition', name='svtrv2')
...
def test(config, val_ops, best_model_path, device='cpu', seed=42): 
    ...
    best_model = SVTRV2Finetuner.load_from_checkpoint(best_model_path, 
...
if __name__ == '__main__': 
    ...
    config['Architecture']['Decoder']['gtc_decoder']['max_len'] = 25
