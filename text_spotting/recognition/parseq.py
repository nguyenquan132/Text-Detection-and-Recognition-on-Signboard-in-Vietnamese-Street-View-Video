...
def get_state_dict(cfg, state_dict): 
    ...
    for p, v in state_dict.items():
        ...
        if 'encoder' in key: 
            ...
            p = 'decoder.' + p
            if 'embedding' in p or 'head' in p:
                print(f"[SKIP] {p}") 
                continue 
            else:
                new_state_dict[p] = v
...
class PARSeqFinetuner(pl.LightningModule):
    def __init__(self, model, criterion=None, cfg_optimizer=None, cfg_lr_scheduler=None, train_dataloader=None, val_dataloader=None,
        ...
        super(PARSeqFinetuner, self).__init__()
        self.model = model
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % self.metrics_interval == 0:
            ...
            self.train_gt.clear()
            metrics = {'train_loss': loss_dict['loss'],
...
def train(config, train_ops, val_ops, pretrained_model_path, max_epochs, device='cpu', seed=42):
    ...
    train_sampler = RatioSampler(data_source=train_dataset, scales=[[128, 32]],
                     first_bs=32, fix_bs=False, divided_factor=[4,8], is_training=True, seed=seed)
    val_sampler = RatioSampler(data_source=val_dataset, scales=[[128, 32]],
                        first_bs=16, fix_bs=False, divided_factor=[4,8], is_training=False, seed=seed)
    ...
    new_state_dict = get_state_dict(config, checkpoint)
    model.load_state_dict(new_state_dict, strict=False)
    model = model.to(device)

    parseq_finetuner = PARSeqFinetuner(model=model, criterion=criterion, cfg_optimizer=config['Optimizer'], 
    ...
    wandb_logger = WandbLogger(project='Text Recognition', name='parseq')
...
def test(config, val_ops, best_model_path, device='cpu', seed=42): 
    ...
    best_model = PARSeqFinetuner.load_from_checkpoint(best_model_path,
...
if __name__ == '__main__': 
    ...
    config = copy.deepcopy(Config('src/text_rec/OpenOCR/configs/rec/parseq/vit_parseq.yml').cfg)
    ...
    config['Train']['dataset']['transforms'][2]['ARLabelEncode']['character_dict_path'] = 'charset/vietnamese_charset.txt'
    config['Eval']['dataset']['transforms'][1]['ARLabelEncode']['character_dict_path'] = 'charset/vietnamese_charset.txt'
    config['Train']['dataset']['transforms'][2]['ARLabelEncode']['max_text_length'] = 25
    config['Eval']['dataset']['transforms'][1]['ARLabelEncode']['max_text_length'] = 25
    config['Global']['max_text_length'] = 25
    config['Architecture']['Decoder']['max_label_length'] = 25
    ...
    config['Train']['dataset']['transforms'].pop(3)
    config['Eval']['dataset']['transforms'].pop(2)
